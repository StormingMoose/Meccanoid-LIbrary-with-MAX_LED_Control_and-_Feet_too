...
void MeccaBrain::sendImageToFace(byte image[], int count) {
...
byte MeccaBrain::calculateImageCheckSum(byte data[], int count)
{
  int cs = 0;
  for (int i = 0; i < count; i++)
  {
...
void MeccaBrain :: communicate(){
    ...
    if (inputByte == 0x05 && moduleType[moduleNum] == '_'){   // Storming Moose 2024 add the 5 for the new motor        
      outputByte[moduleNum] = 0xE4;                           // if received back 0x05 (module ID is a M.A.X. Motor SM-03), then change servo color to Blue
    	moduleType[moduleNum] = 'M';                            //  this sets the module type, we change things above too
    }

  if (inputByte == 0x06 && moduleType[moduleNum] == '_'){   // Storming Moose 2024 add the F for the new 8X16 LED       
     // outputByte[moduleNum] = 0xE4;                           // if received back 0x06 (module ID is a M.A.X. Face Led), then change face to Happy
    	moduleType[moduleNum] = 'F';                            //  this sets the module type, we change things above too
    }                                                         // not neccessary maybe
...
void MeccaBrain :: sendByte(byte servoData){
  ...
  for (byte mask = 00000001; mask>0; mask <<= 1) {   // iterate through bit mask
    if (servoData & mask){                           // if bitwise AND resolves to true
      ...
      digitalWrite(_pwmPin,LOW);               // send 0
    }
...
byte MeccaBrain :: receiveByte(){
  ...
  for (byte mask = 00000001; mask>0; mask <<= 1) {   // iterate through bit mask
    ...
    if (pulseIn(_pwmPin, HIGH, 2500) > 400){
